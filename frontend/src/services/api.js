...
// Dev: use the local Vite proxy (or VITE_API_BASE_URL override).
// Production: relative (same-origin) by default, but honor VITE_API_BASE_URL
// when set — needed when the frontend is hosted separately from the API
// (e.g. Cloudflare/GitHub Pages frontend + Render backend).
...
// In dev: connect to localhost (or VITE_API_BASE_URL).
// In production: use VITE_WS_URL env var if set, otherwise derive from current page origin
// so the app works on any domain without hardcoded IPs.
const WS_BASE_URL = import.meta.env.DEV
  ? (import.meta.env.VITE_API_BASE_URL || 'ws://localhost:8000').replace(/^https?:\/\//, (m) => m === 'https://' ? 'wss://' : 'ws://')
