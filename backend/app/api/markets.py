        ...
        rows = await conn.fetch("""
            WITH latest AS (
                SELECT DISTINCT ON (market_id)
                    market_id, event_id, predictive_score
            ...
            ORDER BY MAX(s.resolved_at) DESC NULLS LAST
            LIMIT $1
        """, limit)
        return [dict(r) for r in rows]
...
async def list_events(
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        # Dashboard shows: active events + resolved events within last 30 days
        lifecycle_filter = """
            AND (
                lifecycle_status = 'active'
                OR lifecycle_status IS NULL
                OR (lifecycle_status = 'resolved' AND resolved_at > NOW() - INTERVAL '30 days')
            )
        ...
        if search:
            rows = await conn.fetch(f"""
                ...
                AND s.lifecycle_status != 'archived'
                AND (s.event_title ILIKE $1 OR s.title ILIKE $1)
                GROUP BY s.event_id, s.event_title, s.event_slug
